...
export const mobileApi = {
  ...
  getDashboard(baseUrl: string, token: string) {
    return request<ClientDashboardDto>(baseUrl, "/api/client/dashboard", { token });
  },
  getOrders(baseUrl: string, token: string) {
    return request<ClientOrderListItemDto[]>(baseUrl, "/api/client/orders", { token });
  },
  getOrderDetails(baseUrl: string, token: string, orderId: number) {
    return request<ClientOrderDetailsDto>(baseUrl, `/api/client/orders/${orderId}`, { token });
  },
  getOrderAttachments(baseUrl: string, token: string, orderId: number) {
    return request<ClientAttachmentDto[]>(baseUrl, `/api/client/orders/${orderId}/attachments`, { token });
  },
  getReservations(baseUrl: string, token: string) {
    return request<ClientReservationListItemDto[]>(baseUrl, "/api/client/reservations", { token });
  },
  getReservationDetails(baseUrl: string, token: string, reservationId: number) {
    return request<ClientReservationDetailsDto>(baseUrl, `/api/client/reservations/${reservationId}`, { token });
  },
  getNotifications(baseUrl: string, token: string, take = 20) {
    return request<ClientNotificationDto[]>(baseUrl, `/api/client/notifications?take=${take}`, { token });
  },
  ...
  getPages(baseUrl: string) {
    return request<MobilePageListItemDto[]>(baseUrl, "/api/mobile/content/pages");
  },
  getPage(baseUrl: string, slug: string) {
    return request<MobilePageDetailsDto>(baseUrl, `/api/mobile/content/pages/${encodeURIComponent(slug)}`);
  }
};
